    ...
    d3.tsv("data_4_PharmacoDB.tsv", function(data) {
            ...
            if (data[i]["batch"] == batch_select) {
                if (data[i]["time"] == 0) {
                    var new_datapt = {
                        ...
                        pdx_json: [{
                            model: data[i]["model.id"].replace(/\./g,' ').replaceAll(" ", "-"),
                            batch: data[i]["batch"],
                            exp_type: data[i]["exp.type"],
                    ...
                    // TODO: comment line below for all times
                    if (data[i]["time"] <= 70) {
        ...
        for (var i = 0; i < data_formatted.length; i++) {
            ...
            for (var j = 0; j < item.pdx_points[0].volumes.length; j++) {
                data_formatted[i].pdx_points[0].volumes[j] = norm(item.pdx_points[0].volumes[j], first)
...
function batchToggle(plotId, batches) {
    ...
    function onchange() {
        var batch_selected = d3.select('select').property('value')
        $("#pdxplot").remove()
        dataParse(batch_selected)
    };
  }
...
function tumourCurve(data, plotId) {
    // DATA: [{
    //     exp_type: control/treatment,
    //     batch: batch,
    //     model: model,
    //     drug: drug,
    //     pdx_points: [{
    ...
    //         volume: volume
    //     }]
    // }, ...] where each item in the data array is a model
    ...
    var svg = d3.select("#" + plotId)
        .append("svg")
        .attr("id", "pdx" + plotId) // set an id so that I can remove+replace on refresh
