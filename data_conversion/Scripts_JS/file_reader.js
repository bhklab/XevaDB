// This function is used by each of the CSV files and
// will replace any tabs with commas and will produce streams of different CSV files.

const fs = require('fs');

const file_reader = function (files, total_files, file_folder, outputData, streams) {
    files.forEach((file) => {
        ...
        if (typeof (file_folder) === 'string') {
            ...
            file_folder.forEach((folder) => {
                if (folder.includes(file.split('.')[0]) || folder.includes(file.split('_')[0]) || folder.includes(file.split('_')[0].toUpperCase())) {
