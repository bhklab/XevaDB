...
var writableStream = fs.createWriteStream(`../Final_Csv_File/rnasequencing_final.csv`);
                        ...
                        .on('end', () => {
                            ...
                            results.forEach((data) => {
                                ...
                                data.forEach((sequencing) => {
                                            if(geneId!=""){
                                                if(sequencing == geneId) {}
                                                else {
                                                    if(patientId[value]!= " ") {
                                                        csvStream.write({id: id++, gene_id: mapped_data[geneId], sequencing_uid: mapped_data[patientId[value]], value: ((Number(sequencing) === 0) ? Number(sequencing) : Number(sequencing).toFixed(3))});
                                                        value++;
                                                    } else {}
                                                }
                                            }  
