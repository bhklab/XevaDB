...
exports.seed = async (knex) => {
    ...
    await knex('gene_drug_tissue').truncate()
        .on('query', () => {
            console.log(`${'='.repeat(20).trimEnd()}truncating gene_drug_tissue table!${'='.repeat(20).trimEnd()}`);
        })
        .on('query-response', (response) => {
            console.log('truncating gene_drug_tissue table has been completed!', response);
        })
        .on('query-error', (error) => {
            console.log(error, 'an error occurred while creating gene_drug_tissue table!');
        });

    // function to insert data into the copy number variation table.
    const insertData = async (i) => {
        await knex('gene_drug_tissue').insert(seedingData)
            .then(() => {
                if (i) {
                    console.log(`Inserting next 50k rows, currently at index ${i}`);
                } else {
    ...
    // this will streamline the data and seeds the table on every 50k rows
    ...
    await csv(parserParams)
        ...
        .subscribe(async (data, i) => {
            ...
            // insert data for every 50k rows from the input.
        ...
        .then(async () => { // this will insert the remaining data into the table.
            await knex('gene_drug_tissue').insert(seedingData)
                .on('query', () => {
                    console.log('Inserting data into the gene_drug_tissue table!!');
                })
                .on('query-response', (response) => {
                    console.log(`${'='.repeat(20).trimEnd()}Inserted ${response} rows into the gene_drug_tissue table!!${'='.repeat(20).trimEnd()}`);
                })
                .on('query-error', (error) => {
                    console.log(error, 'an error occurred while creating gene_drug_tissue table!');
                });
